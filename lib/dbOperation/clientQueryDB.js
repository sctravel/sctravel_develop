...
//select distinct current_product_daily_price_inventory.date from products inner join current_product_daily_price_inventory on products.product_id = current_product_daily_price_inventory.product_id
//where products.parent_product_id= 1 and products.is_active='Y';


//select distinct DATE_FORMAT(current_product_daily_price_inventory.date,'%Y%m%d') from products inner join current_product_daily_price_inventory on products.product_id = current_product_daily_price_inventory.product_id where products.parent_product_id= 2 and products.is_active='Y';
exports.availableDates = function(parent_product_id,callback) {

    console.info("load children products : "+parent_product_id);

    var sql = "select distinct DATE_FORMAT(current_product_daily_price_inventory.date,'%Y-%m-%d') AS DateStr"+
    " from products inner join current_product_daily_price_inventory on products.product_id = current_product_daily_price_inventory.product_id"+
    " where products.parent_product_id= "+ parent_product_id+ " and products.is_active='Y'";

        //" select count(*) from " + tableNames.productsTable + "AS P "
        //" where parent_product_id= "+parent_product_id + " and is_active='Y'";


   // and current_daily_price_inventory
...
exports.searchChildrenProducts = function(parent_product_id,dates,callback) {
